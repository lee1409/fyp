...
def fcos_losses(
    ...
    bspline_pred = bspline_pred[pos_inds]
    reg_targets = reg_targets[pos_inds]
    bspline_targets = bspline_targets[pos_inds]
    ...
    if pos_inds.numel() > 0:
        ...
        bspline_loss = bspline_pred.sum() * 0
        ctrness_loss = ctrness_pred.sum() * 0

    bspline_loss = F.smooth_l1_loss(
        bspline_pred, bspline_targets, reduction="none")
    bspline_loss = ((bspline_loss.mean(dim=-1) * ctrness_targets).sum()
                    / loss_denorm)
    ...
    losses = {
        ...
        "loss_fcos_bspline": bspline_loss,
    }
    return losses


class BTextOutputs(object):
    def __init__(
            self,
            images,
            locations,
        ...
        self.bspline_pred = bspline_pred
    ...
    def _get_ground_truth(self):
        ...
        bspline_targets = training_targets["bspline_targets"]
        for l in range(len(reg_targets)):
            reg_targets[l] = reg_targets[l] / float(self.strides[l])
            bspline_targets[l] = bspline_targets[l] / float(self.strides[l])
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges):
        ...
        for im_i in range(len(targets)):
            ...
            if bboxes.numel() == 0:
                ...
                bspline_targets.append(locations.new_zeros((locations.size(0), 20)))# bezier_targets.append(locations.new_zeros((locations.size(0), 16)))
                continue
            ...
            bspline_pts = targets_per_im.bspline.view(-1, 10, 2) # bezier_pts = targets_per_im.beziers.view(-1, 8, 2)
            x_targets = bspline_pts[:, :, 0][None] - xs[:, None, None]
            y_targets = bspline_pts[:, :, 1][None] - ys[:, None, None]
            bspline_targets_per_im = torch.stack((x_targets, y_targets), dim=3)
            bspline_targets_per_im = bspline_targets_per_im.view(xs.size(0), bboxes.size(0), 20)# bezier_targets_per_im = bezier_targets_per_im.view(xs.size(0), bboxes.size(0), 16)
            ...
            bspline_targets_per_im = bspline_targets_per_im[range(len(locations)), locations_to_gt_inds]
            ...
            bspline_targets.append(bspline_targets_per_im)
            # target_inds.append(target_inds_per_im)

        return {
            "labels": labels,
            "reg_targets": reg_targets,
            "bspline_targets": bspline_targets}
    ...
    def losses(self):
        ...
        labels, reg_targets, bspline_targets = (
            training_targets["labels"],
            training_targets["reg_targets"],
            training_targets["bspline_targets"])
        ...
        bspline_pred = cat(
            [
                ...
                x.permute(0, 2, 3, 1).reshape(-1, 20)
                for x in self.bspline_pred
            ], dim=0,)
        ctrness_pred = cat(
            [
        ...
        bspline_targets = cat(
            [
                ...
                x.reshape(-1, 20) for x in bspline_targets
            ], dim=0,)
    ...
    def predict_proposals(self, top_feats):
        ...
        for i, instance in enumerate(zip(*bundle.values())):
            ...
            c = instance_dict["c"]
            # top_feat is the bspline regression
    ...
    def forward_for_single_feature_map(
        ...
        for i in range(N):
            ...
            if top_feat is not None:
                ...
                # decentralize
                bspline = per_locations.unsqueeze(1) + per_top_feat.view(-1, 10, 2) # bezier_detections = per_locations.unsqueeze(1) + per_top_feat.view(-1, 8, 2) 
                bspline = bspline.view(-1, 20) # bezier_detections = bezier_detections.view(-1, 20)
                boxlist.top_feat = bspline
